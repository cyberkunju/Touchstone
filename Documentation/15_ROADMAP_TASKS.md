...
| P3.6 | 🔶 A/B verdict ✅ | v6-vs-v5 rec A/B RUN (bench/ab_v6.py, same tap_line code path, det pinned v5, MRZ-exact law over 40 passports/80 lines): **v6-small WINS raw crops — exactRate 0.50 vs 0.25, charAcc 0.72 vs 0.68, 2.9× faster (62.9 vs 179.5 ms/doc)**; verdict artifact bench/baselines/ab-v6-rec.json w/ sha256-pinned candidate artifacts. Enum stays v5-LOCKED pending the browser-side end-to-end gate A/B (change control: raw-crop wins ≠ pipeline wins — beam/lattice interplay must be certified) | A/B result recorded in baselines ✅; enum locked ✅ (at v5, evidence on file) |
...
| P4.3 | 🔶 rulings+cluster+closure ✅ | Table engine: rulings tier + quality stage done. **Tier-2 borderless DONE**: `stages/align_stage.py` x/y-cluster alignment (majority-column law — wrapped lines can't fabricate columns; 60% occupancy — prose never becomes a table; method='cluster' schema-frozen) wired into ladder as rulings fallback. **Brain closure+repair DONE**: `src/docgraph/table-closure.ts`. Remaining: SLANet_plus/LORE neural tier (flag-future, same contract); stamp/seal masks | closure attests clean invoices ✅; repair suite green ✅; cluster tier tests green ✅ |
